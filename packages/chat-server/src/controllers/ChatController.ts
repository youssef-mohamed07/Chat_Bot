    ...
    try {
      ...
      if (message.trim() === '__init__') {
        ...
        if (customerInfo) {
          ...
          console.log(`📧 Attempting to send welcome email to ${customerInfo.email}...`)
          this.emailService.sendWelcomeEmail({
            ...
            if (sent) {
              console.log(`✅ Welcome email sent successfully to ${customerInfo.email}`)
            } else {
              console.warn(`⚠️ Welcome email failed to send to ${customerInfo.email}`)
            }
          }).catch(err => {
      ...
      // Check if this is a button action that should skip smart message handling
      const isButtonAction = message.startsWith('dest:') || 
                             message.startsWith('set_dates:') || 
                             message.startsWith('set_pax:') ||
                             message.startsWith('budget:') ||
                             message.startsWith('hotel:') ||
                             message.startsWith('meal:') ||
                             message.startsWith('room:') ||
                             message.startsWith('ask_') ||
                             message === 'confirm_booking' ||
                             message === 'modify_booking' ||
