...
export const useChatWidget = (apiBaseURL?: string) => {
 ...
 const fullResponse: string = response?.reply ?? ''
 const blocks = response?.ui?.blocks as any[] | undefined
 ...
 try {
 const response = await apiService.sendSupportRequest({
 ...supportData,
 userId,
 lang: lang ?? 'en'
 })
 ...
 const successMessage = response.message || LABELS[lang ?? 'en'].requestSent
 setMessages(prev => [...prev, { 
 text: successMessage, 
 isUser: false, 
 timestamp: new Date() 
 ...
 const errorMessage = response.error || LABELS[lang ?? 'en'].requestFailed
 setMessages(prev => [...prev, { 
 text: errorMessage, 
 isUser: false, 
 timestamp: new Date() 
